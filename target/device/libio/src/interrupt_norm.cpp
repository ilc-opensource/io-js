...
//	void (*callback)(void);		// function pointer
	uv_async_t async_callback;      // async callback
	char * reopen_path;
	uint32_t mode;
...
static void * interrupt_main ( void * pargs )
{
	...
	int max_timeout_msec = *((int *)pargs);
	trace_debug("max_timeout_msec = %d\n", max_timeout_msec);
	...
	while(loop == 1){
		...
		uv_mutex_unlock(&idesc.mutex);

		/* Select on the FD set - infinite timeout */
		struct timeval timeout = {max_timeout_msec / 1000, max_timeout_msec % 1000};
		ret = select(1 + max, &fdset, 0, &fdset_except, &timeout);
		/* Receive bytes */
		switch(ret){
			case -1:
			...
			case 0:
				...
				pinterrupt = idesc.phead;
				while(pinterrupt != NULL){
					uv_close((uv_handle_t *)&(pinterrupt->async_callback), NULL);
					pinterrupt = pinterrupt->pnext;
				}
				trace_debug("interrupt finished as timeout!");
				break;
			default:
				/* Process data */
				...
				while(pinterrupt != NULL){
					...
					if (FD_ISSET(pinterrupt->ihandle, &fdset_except)){
						/* Pin based interrupt */
						// pinterrupt->callback();
						uv_async_send(&pinterrupt->async_callback);
						...
						trace_debug("%s timer interrupt fired!", __func__);
...
void uv_callback(uv_async_t *handle, int status /*UNUSED*/) {
	void (*callback)() = (void (*)())handle->data;
	callback();
}
...
static int addIRQEntry(int ihandle, uint32_t pin, void (*callback)(void), uint32_t mode, int ct_thresh)
{
	...
	pinterrupt = (struct interrupt *)malloc(sizeof(struct interrupt));
	if (pinterrupt == NULL){
	...
	pinterrupt->async_callback.data = (void *)callback;
	uv_async_init(uv_default_loop(), &pinterrupt->async_callback, uv_callback);
	pinterrupt->pin = pin;
...
void uv_irq_loop(uv_work_t *req)
{
	interrupt_main(req->data);
}
 ...
 */
int interrupt_init(int max_timeout_msec)
{
	extern int errno;
	int ret = 0, fd;
	struct hpet_info info;
	...
	uv_mutex_init(&idesc.mutex);

	pthread_barrier_init(&idesc.barrier, 0, 2);

	/* Create IRQ work */
	uv_work_t *req = new uv_work_t();
	req->data = (void *)&max_timeout_msec;

	uv_queue_work(uv_default_loop(), req, uv_irq_loop, NULL);
